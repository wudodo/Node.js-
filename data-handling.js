var data = [
  ...
  {
    "name": "不一样的天空 / What's Eating Gilbert Grape",
    "score": "8.6",
    "ranking": "249",
    "country": "美国",
    "type": [
...
for (var i = 0; i < data.length; i++) {
    ...
    t = t.slice(0, 2)
    type[type.length-1] = t
    // 判断: 如果 "总类型"里没有这个类型, 就加入这个类型(type)
    for (var j = 0; j < type.length; j++) {
        if(!allType.includes(type[j])) {
            allType.push(type[j])
        }
    }
}

// 现在有了存储所有'类型'的数组 allType ,用这里的每个元素作为 key 创建一个object,
// 遍历 type, object[type[i]] += 1
for (var k = 0; k < allType.length; k++) {
    let t = allType[k]
    o[t] = 0
}
